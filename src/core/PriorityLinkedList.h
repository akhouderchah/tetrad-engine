...
public:
	...
	inline LinkedNode<T> *First(size_t priority=0) const{ DEBUG_ASSERT(priority < N);
		DEBUG_ASSERT(m_Initialized);
		if(priority == 0){ return this->HeadToNull(this->m_Head.pNext); }
		return this->HeadToNull(m_pPriorityTail[priority-1]->pNext); }

	inline LinkedNode<T> *Last(size_t priority=N-1) const{ DEBUG_ASSERT(priority < N);
		DEBUG_ASSERT(m_Initialized);
