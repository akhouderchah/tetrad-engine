...
vector<pair<ObjectHandle::version_t, EntityManager::compList_t>> EntityManager::s_EntityList;
...
Entity EntityManager::CreateEntity()
{
	...
	if(s_FreeList.size() <= 1)
	{
		...
		{
			LOG("Failed to allocate space for a new entity! Perhaps remove some existing entities or expand the ObjectHandle sizes.\n");
...
IComponent *EntityManager::AddComponent(Entity entity, ObjectHandle::type_t type, IComponent *pComp, bool skipRefresh)
{
...
void EntityManager::RemoveComponent(Entity entity, ObjectHandle::type_t type, bool skipRefresh)
{
	ObjectHandle::ID_t ID = entity.m_ID.GetID();
	...
	if(displaced != 0)
	{
		...
		ObjectHandle::handle_t dispHandle = ObjectHandle::constructRawHandle(displaced, type, 0u);
